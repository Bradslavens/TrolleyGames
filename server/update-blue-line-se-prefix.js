// update-blue-line-se-prefix.js - Add "S" prefix to Blue Line South East signals
...
// Function to update Blue Line South East signals with "S" prefix
const updateBlueLineSouthEastSignals = () => {
  return new Promise((resolve, reject) => {
    // First, let's see what signals we have for Blue Line South East
    db.all('SELECT * FROM signals WHERE line = ?', ['Blue Line South East'], (err, rows) => {
      if (err) {
        reject(err);
        return;
      }
      
      console.log(`Found ${rows.length} Blue Line South East signals:`);
      rows.forEach(signal => {
      ...
      if (rows.length === 0) {
        console.log('No Blue Line South East signals found to update.');
        resolve();
        return;
      }
      ...
      Promise.all(updatePromises)
        .then(() => {
          console.log('All Blue Line South East signals updated successfully!');
          resolve();
        })
        .catch(reject);
    });
...
const main = async () => {
  try {
    console.log('Starting update of Blue Line South East signals...');
    await updateBlueLineSouthEastSignals();
    ...
    await new Promise((resolve, reject) => {
      db.all('SELECT * FROM signals WHERE line = ?', ['Blue Line South East'], (err, rows) => {
        ...
        console.log(`\nAfter update - Blue Line South East signals:`);
        rows.forEach(signal => {
