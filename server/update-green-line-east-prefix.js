// update-green-line-east-prefix.js - Add "M" prefix to Green Line East signals
...
// Function to update Green Line East signals with "M" prefix
const updateGreenLineEastSignals = () => {
  return new Promise((resolve, reject) => {
    // First, let's see what signals we have for Green Line East
    db.all('SELECT * FROM signals WHERE line = ?', ['Green Line East'], (err, rows) => {
      if (err) {
        reject(err);
        return;
      }
      
      console.log(`Found ${rows.length} Green Line East signals:`);
      rows.forEach(signal => {
      ...
      if (rows.length === 0) {
        console.log('No Green Line East signals found to update.');
        resolve();
        return;
      }
      ...
      Promise.all(updatePromises)
        .then(() => {
          console.log('All Green Line East signals updated successfully!');
          resolve();
        })
        .catch(reject);
    });
...
const main = async () => {
  try {
    console.log('Starting update of Green Line East signals...');
    await updateGreenLineEastSignals();
    ...
    await new Promise((resolve, reject) => {
      db.all('SELECT * FROM signals WHERE line = ?', ['Green Line East'], (err, rows) => {
        ...
        console.log(`\nAfter update - Green Line East signals:`);
        rows.forEach(signal => {
