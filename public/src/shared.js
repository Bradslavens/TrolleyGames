...
export function showMenu(progress, onSelectLevel, { isAdmin = false, onAdmin = null } = {}) {
  ...
  lines.forEach(line => {
    ...
    // Lines without a transcribed schematic have no signal data yet.
    const covered = isCovered(line);
    if (!covered) {
      lineDiv.classList.add('is-coming-soon');
      const note = document.createElement('span');
      note.className = 'line-coming-soon';
    ...
    levels.forEach((level, idx) => {
      ...
      btn.disabled = !covered || (progress[line] == null ? idx !== 0 : idx > progress[line]);
...
export function palette() {
  ...
  return {
    ...
    cream2: v('--card-cream-2', '#e9dcbd'),
    creamOnDark: v('--cream-on-dark', '#f6eedb'),
    ref: v('--ink-soft', '#8a7f66'),
    ink: v('--ink', '#2a2018'),
